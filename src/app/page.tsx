...
export default function MainPage({
  ...
  const tags: Tag[] = useMemo(() => {
    const equip = itemEquipment.reduce(
      (acc, cur) => {
  ...
  const equipments = useMemo(() => {
    const filtered = itemEquipment
      .filter((item) => {
        return (
          !item.item_equipment_page_name.includes('Cash') &&
          !['아케인심볼', '탈것', '이펙트', '안드로이드', '의자'].includes(item.item_equipment_page_name.split(' ')[1])
        );
      })
      .sort((a, b) => {
  ...
  return (
    <main className="z-50 mx-auto flex h-full w-full min-w-64 max-w-3xl flex-col justify-between gap-y-4 overflow-y-auto p-5 pb-28">
        ...
        {basicInfo && (
          <>
            ...
            <div className="flex flex-wrap gap-x-0.5 gap-y-2">
              {tags.map((tag, index) => (
                <Tag key={index} color={tag.color} className="text-base">
                  {tag.label}
                </Tag>
              ))}
            </div>
              ...
              items={[
                {
                  ...
                  children: (
                    <ul className="list-disc pl-4">
                      ...
                      <li>생년월일: {formatDate(basicInfo?.character_date_create)}</li>
                    </ul>
                  ),
                },
              ]}
                    ...
                    <ul className="list-disc pl-4">
                      {equipments.map((item) => (
                        <li key={`item-${item.item_name}`}>
                          ...
                          {item.rating && (
                            <Tag bordered={false} color={COLOR_BY_RATING[item.rating]}>
                              {item.rating}
                            </Tag>
                          )}
                        </li>
              ...
              items={[
                {
                  ...
                  children: (
                    <ul className="list-disc pl-4">
                      {stat.map((stat) => (
                        <li key={`stat-${stat.stat_name}`}>
                          {stat.stat_name}: {Number(stat.stat_value).toLocaleString('ko-KR')}
                        </li>
                      ))}
                    </ul>
                  ),
              ...
              items={[
                {
                  ...
                  children: (
                    <div className="flex flex-col gap-y-4">
                      <div>
                        <label className="font-bold text-blue-500">스킬 프리셋</label>
                        <ul className="list-disc pl-4">
                          {characterInfo?.skillInfo?.skill?.preset.map((preset) => (
                            <div key={`preset-${preset.preset_slot_no}`}>
                              <li>{preset.preset_slot_no}번 프리셋</li>
                              <span>
                                {preset.skill_name_1 || 'X'} /{preset.skill_name_2 || 'X'} /{preset.skill_name_3 || 'X'}{' '}
                        ...
                        {(characterInfo.skillInfo?.skill?.steal_skill.length || 0) > 0 && (
                          <>
                            <br />
                            <label className="font-bold text-blue-500">훔친 스킬</label>
                            <ul className="list-disc pl-4">
                        ...
                        {(characterInfo.skillInfo?.skill?.stella_memorize.length || 0) > 0 && (
                          <>
                            <br />
                            <label className="font-bold text-blue-500">스텔라 메모라이즈</label>
                            <ul className="list-disc pl-4">
                  ...
                  children: (
                    <>
                      {vmatrixInfo?.skill.length > 0 && (
                        <>
                          <label className="font-bold text-blue-500">5차 스킬</label>
                          <ul className="list-disc pl-4">
                            {vmatrixInfo.skill.map((core) => (
                              <li key={`5차-${core.slot_id}`}>
                                {core.v_core_name} ({core.v_core_level}
                                {!!core.slot_level && `+${core.slot_level}`}){' '}
                                {!!core.slot_level && (
                      ...
                      {vmatrixInfo?.enhancement.length > 0 && (
                        <>
                          <label className="font-bold text-blue-500">강화 스킬</label>
                          <ul className="list-disc pl-4">
                            {vmatrixInfo.enhancement.map((core) => (
                              <>
                                <li>
                                  {core.v_core_name} ({core.v_core_level}
                                  {!!core.slot_level && `+${core.slot_level}`}){' '}
                                  ...
                                  {core.v_core_type === 'Enhancement' && (
                                    <>
                                      <br />({core.v_core_skill_name_1}+{core.v_core_skill_name_2}+
                      ...
                      {vmatrixInfo?.special.length > 0 && (
                        <>
                          <label className="font-bold text-blue-500">특수 스킬</label>
                          <ul className="list-disc pl-4">
                            {vmatrixInfo.special.map((core) => (
                  ...
                  label: (
                    <div className="font-semibold">
                      ...
                      <span className="text-sm font-normal">
                        펫 생명이 <span className="font-bold text-blue-500">3일</span> 이하로 남은 경우{' '}
      ...
      <div className="flex flex-col items-center justify-self-end">
        ...
        <span className="text-xs font-light text-gray-400">Data Provided By NEXON</span>
      </div>
    </main>
  );
}
